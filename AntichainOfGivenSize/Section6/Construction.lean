...
import AntichainOfGivenSize.Section6.Survivors
import AntichainOfGivenSize.Section6.StageBridge
import AntichainOfGivenSize.Section6.Address
import AntichainOfGivenSize.Section6.StageArithmetic
import AntichainOfGivenSize.Section6.IndexedBounds
import AntichainOfGivenSize.Section6.NatParameters
import AntichainOfGivenSize.MatchingParameters
...
noncomputable def chooseDigit (q r c : ℕ) : ℕ :=
  Classical.choose (AntichainOfGivenSize.Section6.exists_remainderZ_cast q
    ((-1 : ZMod (2 ^ q)) ^ r * (c : ZMod (2 ^ q))))

theorem chooseDigit_lt (q r c : ℕ) : chooseDigit q r c < 2 ^ q :=
  (Classical.choose_spec (AntichainOfGivenSize.Section6.exists_remainderZ_cast q
    ((-1 : ZMod (2 ^ q)) ^ r * (c : ZMod (2 ^ q))))).1

theorem chooseDigit_cast (q r c : ℕ) :
    (AntichainOfGivenSize.Section6.remainderZ q (chooseDigit q r c) : ZMod (2 ^ q)) =
      (-1 : ZMod (2 ^ q)) ^ r * (c : ZMod (2 ^ q)) :=
  (Classical.choose_spec (AntichainOfGivenSize.Section6.exists_remainderZ_cast q
...
theorem chosenDigit_signed_block {h q r c : ℕ} :
    (-1 : ZMod (2 ^ ((h + 1) * q))) ^ r *
        (AntichainOfGivenSize.Section6.remainderZ q (chooseDigit q r c) : ZMod (2 ^ ((h + 1) * q))) *
        (2 : ZMod (2 ^ ((h + 1) * q))) ^ (h * q) =
      ((2 ^ (h * q) * c : ℕ) : ZMod (2 ^ ((h + 1) * q))) := by
  rw [AntichainOfGivenSize.Section6.pow_block_succ]
  simpa [Nat.cast_pow, mul_comm, mul_left_comm, mul_assoc] using
    (signed_mul_lift (a := 2 ^ (h * q)) (b := 2 ^ q) (r := r)
      (z := AntichainOfGivenSize.Section6.remainderZ q (chooseDigit q r c)) (c := c)
  ...
  ∀ {r q h : ℕ}, 0 < q → (w : Fin r → ℕ) →
    ...
    (((generatedIdealIdx (selected (q := q) Tnew dnew)
      (finiteHistoryGenerator w T D)).card : ℕ) :
        ZMod (2 ^ ((h + 1) * q))) =
      (-1 : ZMod (2 ^ ((h + 1) * q))) ^ r *
        (AntichainOfGivenSize.Section6.remainderZ q dnew : ZMod (2 ^ ((h + 1) * q))) *
        (2 : ZMod (2 ^ ((h + 1) * q))) ^ (h * q)
...
theorem exists_matching_finiteHistory
    ...
    (hsize : ∀ (k : ℕ), k ≤ H →
      ∀ (T : Fin k → Finset (Fin (AntichainOfGivenSize.Section6.addressBits H + 1))) (D : Fin k → ℕ),
        finiteHistoryCard (q := q) (AntichainOfGivenSize.Section6.canonicalWeight H) T D ≤ y) :
    ∃ (T : Fin h → Finset (Fin (AntichainOfGivenSize.Section6.addressBits H + 1))) (D : Fin h → ℕ),
      (∀ g, ∑ j ∈ T g, AntichainOfGivenSize.Section6.canonicalWeight H j = g.val) ∧
      finiteHistoryCard (q := q) (AntichainOfGivenSize.Section6.canonicalWeight H) T D ≡ y
        [MOD 2 ^ (h * q)] := by
  induction h with
  | zero =>
      ...
      · simpa using (Nat.modEq_one :
          finiteHistoryCard (q := q) (AntichainOfGivenSize.Section6.canonicalWeight H) Fin.elim0 Fin.elim0
            ≡ y [MOD 1])
  | succ h ih =>
      have hh' : h ≤ H := by omega
      obtain ⟨T, D, hweights, hmod⟩ := ih hh'
      let m : ℕ := finiteHistoryCard (q := q) (AntichainOfGivenSize.Section6.canonicalWeight H) T D
      let a : ℕ := 2 ^ (h * q)
      let c : ℕ := (y - m) / a
      let d : ℕ := chooseDigit q (AntichainOfGivenSize.Section6.addressBits H + 1) c
      have hhlt : h < H := by omega
      let Tnew : Finset (Fin (AntichainOfGivenSize.Section6.addressBits H + 1)) :=
        AntichainOfGivenSize.Section6.canonicalAddress H ⟨h, hhlt⟩
      let T' := snocFun T Tnew
      let D' := snocFun D d
      have hweightNew :
          ∑ j ∈ Tnew, AntichainOfGivenSize.Section6.canonicalWeight H j = h := by
        simpa [Tnew] using
          (AntichainOfGivenSize.Section6.sum_canonicalAddress_weight hH ⟨h, hhlt⟩)
      have hU :
          (h + 1) * q ≤
            (∑ j : Fin (AntichainOfGivenSize.Section6.addressBits H + 1),
              q * AntichainOfGivenSize.Section6.canonicalWeight H j) +
              ∑ _j : Fin (AntichainOfGivenSize.Section6.addressBits H + 1), q := by
        rw [← Finset.mul_sum, AntichainOfGivenSize.Section6.sum_canonicalWeight hH]
        have hrankPos : 0 < AntichainOfGivenSize.Section6.addressBits H + 1 := by omega
        simp only [Finset.sum_const, Finset.card_univ, Fintype.card_fin,
          nsmul_eq_mul]
        have hhsub : h ≤ H - 1 := by omega
        have hqpart : h * q ≤ (H - 1) * q :=
          Nat.mul_le_mul_right q hhsub
        have hrankOne : 1 ≤ AntichainOfGivenSize.Section6.addressBits H + 1 := by omega
        have hqrank : q ≤ (AntichainOfGivenSize.Section6.addressBits H + 1) * q := by
          simpa using Nat.mul_le_mul_right q hrankOne
        calc
          (h + 1) * q = h * q + q := by rw [Nat.add_mul, one_mul]
          _ ≤ (H - 1) * q + (AntichainOfGivenSize.Section6.addressBits H + 1) * q :=
            Nat.add_le_add hqpart hqrank
          _ = q * (H - 1) + (AntichainOfGivenSize.Section6.addressBits H + 1) * q := by
      ...
      let delta : ℕ :=
        (generatedIdealIdx (selected (q := q) Tnew d)
          (finiteHistoryGenerator (q := q) (AntichainOfGivenSize.Section6.canonicalWeight H) T D)).card
      have hdeltaCast :
          (delta : ZMod (2 ^ ((h + 1) * q))) =
            (-1 : ZMod (2 ^ ((h + 1) * q))) ^ (AntichainOfGivenSize.Section6.addressBits H + 1) *
              (AntichainOfGivenSize.Section6.remainderZ q d : ZMod (2 ^ ((h + 1) * q))) *
              (2 : ZMod (2 ^ ((h + 1) * q))) ^ (h * q) := by
        exact hexact hq (AntichainOfGivenSize.Section6.canonicalWeight H) T D Tnew d hpast hweightNew hU
      have hchosen :
          (-1 : ZMod (2 ^ ((h + 1) * q))) ^ (AntichainOfGivenSize.Section6.addressBits H + 1) *
              (AntichainOfGivenSize.Section6.remainderZ q d : ZMod (2 ^ ((h + 1) * q))) *
              (2 : ZMod (2 ^ ((h + 1) * q))) ^ (h * q) =
        ...
        simpa [d] using
          (chosenDigit_signed_block (h := h) (q := q)
            (r := AntichainOfGivenSize.Section6.addressBits H + 1) (c := c))
      ...
      have hnext : m + delta ≡ y [MOD 2 ^ ((h + 1) * q)] := by
        exact AntichainOfGivenSize.Section6.extend_pow_modEq_digit hmle hmod hc hdelta
      refine ⟨T', D', ?_, ?_⟩
      · intro g
...
theorem finiteHistoryCard_lt_of_bounds
    ...
    (hn : 2 ^ N ≤ n)
    (T : Fin h → Finset (Fin (AntichainOfGivenSize.Section6.addressBits H + 1))) (D : Fin h → ℕ) :
    finiteHistoryCard (q := q) (AntichainOfGivenSize.Section6.canonicalWeight H) T D < n := by
  let r := AntichainOfGivenSize.Section6.addressBits H + 1
  have hr : r ≤ q := by
    exact AntichainOfGivenSize.Section6.addressRank_le_of_le_two_pow hH hHpow
  unfold finiteHistoryCard finiteHistoryGenerator
  apply AntichainOfGivenSize.generatedIdealIdx_card_lt_of_exponent_bounds
      (generatorExponent := q)
      (universeExponent := H * q + 2 * q ^ 2 + H)
      (N := N)
  · rw [Finset.card_univ, card_blockIndex]
    exact AntichainOfGivenSize.Section6.generatorCount_le_two_pow hr hq4
  · rw [card_atomVertex, AntichainOfGivenSize.Section6.sum_canonicalWeight hH]
    calc
      q * (H - 1) + r * q + q ^ 2 + h ≤
          q * (H - 1) + q * r + q ^ 2 + H := by
        rw [Nat.mul_comm r q]
        omega
      _ ≤ H * q + 2 * q ^ 2 + H := AntichainOfGivenSize.Section6.universeSize_le hr
...
theorem exists_concrete_nat_matching
    ...
    ∃ m : ℕ,
      0 < m ∧ m < n ∧ m ≡ n + 1 [MOD 2 ^ (H * q)] ∧
        AntichainOfGivenSize.alpha m ≤
          2 * (AntichainOfGivenSize.Section6.addressBits H + 1) + 2 * q := by
  have hq : 0 < q := by omega
  have hsize : ∀ (k : ℕ), k ≤ H →
      ∀ (T : Fin k → Finset (Fin (AntichainOfGivenSize.Section6.addressBits H + 1))) (D : Fin k → ℕ),
        finiteHistoryCard (q := q) (AntichainOfGivenSize.Section6.canonicalWeight H) T D ≤ n + 1 := by
    intro k hk T D
  ...
  let m := finiteHistoryCard (q := q) (AntichainOfGivenSize.Section6.canonicalWeight H) T D
  refine ⟨m, ?_, ?_, ?_, ?_⟩
  · unfold m finiteHistoryCard
    apply AntichainOfGivenSize.generatedIdealIdx_card_pos
  ...
  · unfold m finiteHistoryCard finiteHistoryGenerator
    simpa [card_blockIndex, Nat.mul_comm] using
      (AntichainOfGivenSize.alpha_generatedIdealIdx_univ_card_le
        (oldGenerator (q := q) (AntichainOfGivenSize.Section6.canonicalWeight H) T D))

theorem constructionMatching_of_exact (hexact : ExactStageIncrement) :
    AntichainOfGivenSize.MatchingSuccessorAtBits 10 8 16
      AntichainOfGivenSize.constructionBits := by
  intro x hx n hrange
  let N : ℕ := AntichainOfGivenSize.binaryLog n
  let q : ℕ := ⌈x⌉₊
  let H : ℕ := AntichainOfGivenSize.Section6.stageCount N q
  ...
  have hlogN : Real.logb 2 (n : ℝ) < (N : ℝ) + 1 := by
    ...
    have heq : ⌊Real.logb 2 (n : ℝ)⌋₊ = N := by
      simpa [N, AntichainOfGivenSize.binaryLog] using
  ...
  have hpowPredN : 2 ^ (q - 1) ≤ N := by
    ...
    omega
  have hHtwo : 2 ≤ H := by
    simpa [H, AntichainOfGivenSize.Section6.stageCount] using
      (AntichainOfGivenSize.Section6.two_le_stageCount_of_pow_pred_le (by omega) hpowPredN)
  have hH : 1 < H := by omega
  have hsmall : 4 * q ^ 2 ≤ N :=
    (AntichainOfGivenSize.Section6.four_sq_le_two_pow_pred (by omega)).trans hpowPredN
  have hHlNat : H * q ≤ N := by
    calc
      H * q ≤ H * (q + 2) := by gcongr; omega
      _ ≤ N - 4 * q ^ 2 := by
        simpa [H] using AntichainOfGivenSize.Section6.stageCount_mul_le N q
      _ ≤ N := Nat.sub_le _ _
  ...
  have hNlog : (N : ℝ) ≤ Real.logb 2 (n : ℝ) := by
    simpa [N] using
      AntichainOfGivenSize.binaryLog_le_logb_of_wideRange 10 x n hrange
  have hNupper : (N : ℝ) < (2 : ℝ) ^ x * x / 1024 := by
    calc
      (N : ℝ) ≤ Real.logb 2 (n : ℝ) := hNlog
      _ < (2 : ℝ) ^ (x + Real.logb 2 x - 10) := hrange.2
      _ = (2 : ℝ) ^ x * x / (2 : ℝ) ^ (10 : ℝ) :=
        AntichainOfGivenSize.wideRange_upper_endpoint 10 x hxpos
      _ = (2 : ℝ) ^ x * x / 1024 := by
        norm_num [Real.rpow_natCast]
  ...
  have hexponents : H * q + 2 * q ^ 2 + H + q < N := by
    simpa [H] using AntichainOfGivenSize.Section6.constructionExponent_lt (by omega) hsmall
  have hn0 : n ≠ 0 := by
    intro hnzero
    subst n
    ...
    linarith
  have hnPow : 2 ^ N ≤ n := by
    simpa [N, AntichainOfGivenSize.binaryLog] using Nat.pow_log_le_self 2 hn0
  ...
  · simpa [AntichainOfGivenSize.constructionBits, AntichainOfGivenSize.Section6.matchingBits, N, q, H]
      using hmod
  · have hrank : AntichainOfGivenSize.Section6.addressBits H + 1 ≤ q :=
      AntichainOfGivenSize.Section6.addressRank_le_of_le_two_pow hH hHpow
    have hcount : 2 * (AntichainOfGivenSize.Section6.addressBits H + 1) + 2 * q ≤ 4 * q := by
      omega
    have hcountReal :
        ((2 * (AntichainOfGivenSize.Section6.addressBits H + 1) + 2 * q : ℕ) : ℝ) ≤ 4 * (q : ℝ) := by
      exact_mod_cast hcount
    have hqBound : 4 * (q : ℝ) ≤ 8 * x := by
      have : (q : ℝ) < x + 1 := hqx
      nlinarith
    calc
      (AntichainOfGivenSize.alpha m : ℝ) ≤
          ((2 * (AntichainOfGivenSize.Section6.addressBits H + 1) + 2 * q : ℕ) : ℝ) := by
